...
//placeHolder的左侧距离
static const NSInteger placeHolderLeft = 0;
...
#pragma mark - 初始化
//- (instancetype)initWithPlaceHolder:(NSString*)placeHolder withSeparateCount:(NSInteger)count withFont:(UIFont *)font
//{
//    return [self initWithFrame:CGRectZero withPlaceHolder:placeHolder withSeparateCount:count withFont:font];
//}

- (instancetype)initWithFrame:(CGRect)frame withPlaceHolder:(NSString*)placeHolder withSeparateCount:(NSInteger)count withFont:(UIFont *)font
{
...
//        self.tintColor = [UIColor redColor];//光标颜色
//
//        //左侧填充视图
//        UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, placeHolderLeft, frame.size.height)];
//        self.leftView = view;
...
- (void)loadCustomViewWithPlaceHolder:(NSString*)placeHolder
{
    if (placeHolder) {
        ...
        _placeHolderLabel.textColor = [UIColor colorWithR:204 G:204 B:204 A:1];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if ([string isEqualToString:@""] && range.location == 0 && range.length == 1 && textField.text.length == 1) {
