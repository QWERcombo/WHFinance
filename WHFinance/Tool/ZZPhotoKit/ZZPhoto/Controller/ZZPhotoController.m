...
@property(strong,nonatomic) TBNavigationController *photoListNavigationController;
...
-(TBNavigationController *)photoListNavigationController{
    if (!_photoListNavigationController) {
        _photoListNavigationController = [[TBNavigationController alloc]initWithRootViewController:self.photoListController];
    }
        ...
        [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status){
            ...
            if (status == PHAuthorizationStatusAuthorized){
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self showController:controller withType:type result:result];
                });
            }
        }];
...
-(void)showController:(UIViewController *)controller withType:(ZZPhotoTypes)type result:(ZZPhotoResult)result
{
    //授权完成，打开相册
    ...
    self.photoListController.nameTpye = SDOUBLE(type);
    ...
    //来自哪个控制器
    self.photoPickerController.actionName = SDOUBLE(type);
...
-(void)showPhotoPicker:(TBNavigationController *)navigationController
{
